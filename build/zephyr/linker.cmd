...
MEMORY
    ...
    RAM (wx) : ORIGIN = (603979776), LENGTH = (524288)
    ITCM ( rw ) : ORIGIN = (0), LENGTH = (65536) DTCM ( rw ) : ORIGIN = (536870912), LENGTH = (131072) EXTMEM ( rw ) : ORIGIN = (2415919104), LENGTH = (268435456) SRAM0 ( rw ) : ORIGIN = (603979776), LENGTH = (524288) SRAM1 ( rw ) : ORIGIN = (805306368), LENGTH = (131072) SRAM2 ( rw ) : ORIGIN = (805437440), LENGTH = (131072) SRAM4 ( rw ) : ORIGIN = (939524096), LENGTH = (65536) SRAM3 ( rw ) : ORIGIN = (805568512), LENGTH = (32768)
...
device_api_area : SUBALIGN(4)
{
 ...
 _mux_control_driver_api_list_start = .; KEEP(*(SORT_BY_NAME(._mux_control_driver_api.static.*))); _mux_control_driver_api_list_end = .;;
 _mux_control_driver_api_ext_end = .;
...
eth_stm32 (NOLOAD) : ALIGN_WITH_INPUT
{
    . = ABSOLUTE((805568512));
    *(.eth_stm32_desc)
    . = ABSOLUTE((805568512)) + 256;
    *(.eth_stm32_buf)
    . = ABSOLUTE((805568512)) + 16K;
} > "SRAM3" AT > "SRAM3"
/DISCARD/ :
{
 KEEP(*(.irq_info*))
 ...
 KEEP(*(.gnu.attributes))
    ...
    ITCM (NOLOAD) : { __ITCM_start = .; KEEP(*(ITCM)) KEEP(*(ITCM.*)) __ITCM_end = .; } > ITCM __ITCM_size = __ITCM_end - __ITCM_start; __ITCM_load_start = LOADADDR(ITCM); DTCM (NOLOAD) : { __DTCM_start = .; KEEP(*(DTCM)) KEEP(*(DTCM.*)) __DTCM_end = .; } > DTCM __DTCM_size = __DTCM_end - __DTCM_start; __DTCM_load_start = LOADADDR(DTCM); EXTMEM (NOLOAD) : { __EXTMEM_start = .; KEEP(*(EXTMEM)) KEEP(*(EXTMEM.*)) __EXTMEM_end = .; } > EXTMEM __EXTMEM_size = __EXTMEM_end - __EXTMEM_start; __EXTMEM_load_start = LOADADDR(EXTMEM); SRAM0 (NOLOAD) : { __SRAM0_start = .; KEEP(*(SRAM0)) KEEP(*(SRAM0.*)) __SRAM0_end = .; } > SRAM0 __SRAM0_size = __SRAM0_end - __SRAM0_start; __SRAM0_load_start = LOADADDR(SRAM0); SRAM1 (NOLOAD) : { __SRAM1_start = .; KEEP(*(SRAM1)) KEEP(*(SRAM1.*)) __SRAM1_end = .; } > SRAM1 __SRAM1_size = __SRAM1_end - __SRAM1_start; __SRAM1_load_start = LOADADDR(SRAM1); SRAM2 (NOLOAD) : { __SRAM2_start = .; KEEP(*(SRAM2)) KEEP(*(SRAM2.*)) __SRAM2_end = .; } > SRAM2 __SRAM2_size = __SRAM2_end - __SRAM2_start; __SRAM2_load_start = LOADADDR(SRAM2); SRAM4 (NOLOAD) : { __SRAM4_start = .; KEEP(*(SRAM4)) KEEP(*(SRAM4.*)) __SRAM4_end = .; } > SRAM4 __SRAM4_size = __SRAM4_end - __SRAM4_start; __SRAM4_load_start = LOADADDR(SRAM4); SRAM3 (NOLOAD) : { __SRAM3_start = .; KEEP(*(SRAM3)) KEEP(*(SRAM3.*)) __SRAM3_end = .; } > SRAM3 __SRAM3_size = __SRAM3_end - __SRAM3_start; __SRAM3_load_start = LOADADDR(SRAM3);
.last_section :
{
  KEEP(*(.last_section))
} > FLASH
...
noinit (NOLOAD) :
{
    ...
    __kernel_ram_end = (603979776) + (524288);
