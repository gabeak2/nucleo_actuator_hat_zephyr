...
static int get_current_ma(uint8_t dac_ch, float *current_ma)
{
	...
	float avg_raw = (float)sum / 10000.0f;
...
/* Shell Command: dac set <channel> <value> */
static int cmd_dac_set(const struct shell *sh, size_t argc, char **argv)
{
	...
	shell_print(sh, "Channel %d set to %d", channel, value);
	return 0;
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_dac,
	...
	SHELL_CMD_ARG(status, NULL, "Read current: status [<ch>]", cmd_dac_status, 1, 1),
...
int main(void)
{
	...
	for (int i = 0; i < 8; i++) {
		...
		if (ret < 0) LOG_WRN("DAC CH %d init failed", i);
	}

	/* Initialize all ADC channels used for current sensing */
	for (int i = 0; i < 8; i++) {
		ret = setup_adc_channel(dac_to_adc_map[i]);
		if (ret < 0) LOG_WRN("ADC CH %d init failed", dac_to_adc_map[i]);
	}
